...
| `layout_reader`            | `"odl"`               | Düzen okuyucusu. `"odl"`: OpenDataLoader (Java). `"liteparse"`: LiteParse (Java'sız, sayfa başına ~0.03 sn, `pip install liteparse`); font ve puntoyu metin katmanından alır, bu yüzden punto eşikleri iki okuyucuda aynıdır. Bilinen sınırları: caption'ı tablo başlığına yutabilir (metin kaybolur), `Figure` caption'larını ayrı tür vermez, ince/geniş kimi görselleri görmez. Yanlış ad bulunursa çıkarım durur. |
