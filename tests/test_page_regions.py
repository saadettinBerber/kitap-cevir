...
def _element(name, strip):
    """strip: (üst, alt) kenarları; öğe sayfa genişliğindedir."""
    top, bottom = strip
    return element(name, (LEFT, top, RIGHT, bottom))


def _region(kind, strip):
    top, bottom = strip
    return Region({"y0": top, "y1": bottom}, {"type": kind, "name": f"{kind}@{top}"})
